...
View(mpg)
ggplot(data = mpg) + # data to plot # "add" geometry
  geom_point(mapping = aes(x = displ, y = hwy, color = class), size = 1.45) # geometric object(points)

ggplot(data = mpg) +
  geom_bar(mapping = aes(x = class)) # count is automatic

ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy)) +
  geom_smooth(mapping = aes(x = displ, y = hwy), color = "blue", se=FALSE) # line of best fit
...
ggplot(data = diamonds) +
  geom_histogram(aes(x = price, fill = clarity, binwidth = 1000))
